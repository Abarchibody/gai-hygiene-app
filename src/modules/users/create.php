...
$title = 'Nouvel Utilisateur - GAI';
$pageTitle = 'Nouvel Utilisateur';
$breadcrumb = '<a href="/" class="hover:text-gai-blue">Accueil</a> / <a href="index.php" class="hover:text-gai-blue">Utilisateurs</a> / <span class="text-gray-800">Nouveau</span>';
...
<?php if ($success): ?>
    <div class="bg-green-50 border-l-4 border-green-400 p-4 mb-6">
        <div class="flex items-center">
            ...
            <div>
                <p class="text-green-800 font-medium">Utilisateur créé avec succès!</p>
                <p class="text-green-600 text-sm mt-1">
                    <a href="index.php" class="underline hover:no-underline">Voir la liste des utilisateurs</a>
                </p>
            </div>
        </div>
    </div>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        ...
        <p class="text-sm text-gray-600 mt-1">Remplissez les informations ci-dessous pour créer un nouvel utilisateur</p>
    </div>

    <!-- Corps du formulaire -->
    <form method="POST" class="p-6">
        <!-- Type d'utilisateur - En premier pour guider la saisie -->
        <div class="mb-6">
            ...
            <div class="grid grid-cols-3 gap-3">
                <label class="relative flex items-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                    <input type="radio" name="type_utilisateur" value="Élève" 
                           <?= ($_POST['type_utilisateur'] ?? '') == 'Élève' ? 'checked' : '' ?>
                           class="sr-only peer" required>
                    <div class="flex items-center w-full relative z-10">
                        <i data-lucide="graduation-cap" class="w-5 h-5 text-blue-500 mr-3 peer-checked:text-blue-600"></i>
                        <div>
                            <div class="font-medium text-gray-900 peer-checked:text-blue-900">Élève</div>
                            <div class="text-xs text-gray-500 peer-checked:text-blue-700">Étudiant du complexe</div>
                        </div>
                    </div>
                ...
                <label class="relative flex items-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                    <input type="radio" name="type_utilisateur" value="Parent" 
                           <?= ($_POST['type_utilisateur'] ?? '') == 'Parent' ? 'checked' : '' ?>
                           class="sr-only peer" required>
                    <div class="flex items-center w-full relative z-10">
                        <i data-lucide="heart" class="w-5 h-5 text-green-500 mr-3 peer-checked:text-green-600"></i>
                        <div>
                            <div class="font-medium text-gray-900 peer-checked:text-green-900">Parent</div>
                            <div class="text-xs text-gray-500 peer-checked:text-green-700">Responsable d'élève</div>
                        </div>
                    </div>
                ...
                <label class="relative flex items-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                    <input type="radio" name="type_utilisateur" value="Enseignant" 
                           <?= ($_POST['type_utilisateur'] ?? '') == 'Enseignant' ? 'checked' : '' ?>
                           class="sr-only peer" required>
                    <div class="flex items-center w-full relative z-10">
                        <i data-lucide="user-check" class="w-5 h-5 text-purple-500 mr-3 peer-checked:text-purple-600"></i>
                        <div>
                            <div class="font-medium text-gray-900 peer-checked:text-purple-900">Enseignant</div>
                            <div class="text-xs text-gray-500 peer-checked:text-purple-700">Personnel éducatif</div>
                        </div>
                    </div>
        ...
        <div class="grid md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="email" name="email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>"
                       ...
                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gai-blue focus:border-transparent transition-colors">
                <p class="text-xs text-gray-500 mt-1">Optionnel - pour les notifications</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                ...
                <input type="tel" name="telephone" value="<?= htmlspecialchars($_POST['telephone'] ?? '') ?>"
                       ...
                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gai-blue focus:border-transparent transition-colors">
                <p class="text-xs text-gray-500 mt-1">Optionnel - pour les contacts d'urgence</p>
            </div>
        </div>
